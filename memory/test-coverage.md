...
Last updated: 2026-05-03. Three claims in the original draft turned out not to match the actual code (`shared/event.py` was not a pub/sub bus, `shared/windows_exec.py` does not construct argv, `transcode/recommend.py` is dead code) and were corrected after agents verified each one against the source. Tasks below are now grounded in real file:line locations.

Baseline: **911 tests passing, 1 skipped** in ~110s on MAIN (the 1 skipped is platform-conditional non-Windows case-sensitive-dedup in `test_transcode_planner_edge_cases.py`). ~12s of overhead is from `tests/test_rip_parser.py` (each test waits ~1s for `queue.get(timeout=1.0)` to drain — unavoidable when testing the real threading path). ~34s comes from `tests/test_pipeline_state_trajectory.py` (9 tests — each runs the full `run_smart_rip` flow with real threading). Newer files are mostly sub-second: organize workflow ~0.9s, sm audit ~2.6s, disk-space pre-checks ~12s (workflow-integration tests run real flows). Pyright strict mode: 8292 baseline errors (mostly missing annotations, treated as a trend rather than a gate).

**Test count history this audit cycle**:
802 (state-trajectory) → 825 (UX/A11y batches start) → 834 → 839 → 846 → 864 (friendly_error) → 878 (planner edge cases) → 890 (organize workflow) → 899 (SM audit) → **911** (disk-space pre-checks).
...
- **Cross-workflow SM behavior audit** (2026-05-03): `tests/test_workflow_sm_audit.py` (9 tests) — uses spies on `_state_transition` / `_state_fail` / `_reset_state_machine` / `sm.complete` to pin the **current** SM contract for each non-`run_smart_rip` workflow. **Audit finding**: only `run_smart_rip` walks the full SM. `_run_disc_inner` (manual TV/Movie) resets at start and forces `sm.complete()` at end but skips intermediate transitions. `run_dump_all` and `run_organize` do NOT touch the SM at all — a prior `FAILED` state leaks into the next run. The cross-cutting criterion in `docs/workflow-stabilization-criteria.md` cannot be ticked until either the code is updated or the criterion is rewritten to acknowledge the SM-free flows. Tests pin the **current** behavior so any future change is a deliberate test update.
- **Organize workflow** (2026-05-03): `tests/test_organize_workflow.py` (12 tests) — closes the §4 test-coverage gap from `workflow-stabilization-criteria.md` (Organize was previously the thinnest-tested workflow). Covers 2 happy paths (movie + TV folder structures), 3 cancellation paths, 2 empty-input paths, the media-type validation loop, abort-with-failed-move, the temp-root safety property (preserves source folders OUTSIDE temp_root even with `opt_auto_delete_temp=True`), and recursive-vs-flat glob patterns. Real find: controller log lines carry a `[HH:MM:SS] ` timestamp prefix, so `==` assertions on log strings need `endswith()`.
- **Disk-space pre-checks** (2026-05-03): `tests/test_disk_space_pre_checks.py` (12 tests, 6 engine-level + 6 workflow-integration) — closes the cross-cutting criterion *"Disk-space pre-checks fire before destructive work"*. Engine-level: `check_disk_space` block/warn/ok decision tree, path-missing fallback, `disk_usage` exception fallback, custom `opt_hard_block_gb` override. Workflow-integration on `run_smart_rip`: check fires before `run_job` (call-order spy), `opt_scan_disc_size=False` bypasses, "block" stops with friendly dialog, "warn"+`opt_warn_low_space=True` prompts user, user-decline stops, `opt_warn_low_space=False` silences prompt. **Audit finding**: the temp-space pre-check is gated by `opt_scan_disc_size`, NOT `opt_check_dest_space` — the latter gates the engine-level destination-space check during `move_files`. The criteria doc note now documents this distinction.
...
The Active list is now **only** the PySide6 migration entry (gated on workflow stabilization). Code-only test gaps from the audit cycle are closed; remaining stabilization work is interactive (real-disc validation per `docs/workflow-stabilization-criteria.md`).

Done in this audit cycle: state machine table tests, event system contract, controller state-machine wiring (+ 2 audit follow-ups), rip stdout-parser, scan stdout-parser (incl. `_parse_drive_info` tri-state quirk pin), `windows_exec.py` path-trust contract, `gui/main_window.py` formatters (incl. test-isolation gotcha), controller pipeline state-trajectory through real flows (incl. `hard_fail` status quirk pin), unused `transcode/` audit, transcode planner edge cases, organize workflow (12 tests covering happy paths + failure modes), cross-workflow SM behavior audit (9 tests + audit finding), disk-space pre-checks (12 tests + criterion misalignment finding).

Workflow-stabilization-criteria checkboxes ticked from this work: §4 organize test-coverage gap, cross-cutting disk-space pre-checks. Cross-cutting "SM reaches terminal state" intentionally left unticked with audit table — gap is honest visibility, not false-green. Remaining open: abort propagation, resume support integration, no-data-loss-on-cancel, behavior-first coverage audit per workflow.
